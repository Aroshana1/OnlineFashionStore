...
function createUser($conn, $fname, $lname, $dob, $gender, $email, $phone, $pwd, $address, $city, $postalcode)
{
    $sql = "INSERT INTO Users(FirstName, LastName, DOB, Gender, Email, Phone, Password, Address, City, PostalCode) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    $stmt = $conn->prepare($sql);
    if (!$stmt) {
        header("Location:../signup.php?error=stmtfailed");
        exit();
    ...
    $stmt->bind_param("ssssssssss", $fname, $lname, $dob, $gender, $email, $phone, $hashedPwd, $address, $city, $postalcode);
    $stmt->execute();
    $stmt->close();
