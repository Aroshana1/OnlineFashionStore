...
<div class = "user-header">
 ...
 //get product and wishlist deatils for relevent user         
...
if ($wishresult->num_rows> 0)
    {
        ...
        <td id ="availability" style ="color:orange;"><?php echo $wish["Availability"] ?></td>
        <td>
          <div id="view-btn">
          ...
          <div id="edit-btn">
          <a id = "addcart-link" href="addtocart.php?productid=<?php echo $wish['ProductID']."&userid=".$wish['UserID']?>"><button id="edit-btn">Add To Cart</button></a>

        </div>
         

            <!--button disabale according to the product availability-->
            <script>
                      window.addEventListener('DOMContentLoaded', function(){
                        var availability = document.getElementById("availability").textContent;
                        var addtocartlink = document.getElementById("addcart-link");
                        var addtocartBtn = document.getElementById("edit-btn");

                        if (availability !== 'InStock') {
                          addtocartlink.removeAttribute('href');
                          addtocartBtn.disabled = true;
                          document.getElementById("status").innerHTML = "Product Out Of Stocks!";
                        }
                      });
              </script>
